    ...
    for (const channel in channels) {
        try {
            ...
            epg[channels[channel]] = json.default.filter(dayEPG => dayEPG.day >= weekday).concat(json.default.filter(dayEPG => dayEPG.day < weekday)).flatMap((dayEPG, index) => dayEPG.entries.map(entry => {
                let startTime = DateTime.now().startOf("day").plus({ days: index }).set({ hour: entry.startTime.split(":")[0], minutes: entry.startTime.split(":")[1] }).setZone("Europe/Rome").minus({ hours: 2 });
                let endTime = DateTime.now().startOf("day").plus({ days: index }).set({ hour: entry.endTime.split(":")[0], minutes: entry.endTime.split(":")[1] }).setZone("Europe/Rome").minus({ hours: 2 });
